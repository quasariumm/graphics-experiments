...
export std::uint8_t GetChannelCount(const DXGI_FORMAT fmt)
{
	switch (fmt)
	{
	// ---------------------------------------------------------
...
export std::size_t BitsPerPixel(const DXGI_FORMAT format)
{
	switch (format)
	{
	case DXGI_FORMAT_R32G32B32A32_TYPELESS:
...
export bool IsUAVCompatible(ID3D12Device2* device, const bool typedUAVLoadAdditionalFormats, const DXGI_FORMAT format) noexcept
{
	...
	case DXGI_FORMAT_B4G4R4A4_UNORM:
		...
		if (typedUAVLoadAdditionalFormats)
		{
			...
			{
				const std::uint32_t mask = D3D12_FORMAT_SUPPORT2_UAV_TYPED_LOAD | D3D12_FORMAT_SUPPORT2_UAV_TYPED_STORE;
...
export bool IsBGR(const DXGI_FORMAT format) noexcept
{
	switch (format)
	{
	case DXGI_FORMAT_B8G8R8A8_UNORM:
...
export bool IsSRGB(const DXGI_FORMAT format) noexcept
{
	switch (format)
	{
	case DXGI_FORMAT_R8G8B8A8_UNORM_SRGB:
...
DXGI_FORMAT ConvertSRVtoResourceFormat(const DXGI_FORMAT format) noexcept
{
	switch (format)
	{
	case DXGI_FORMAT_R32G32B32A32_FLOAT:
