...
DeviceResources::DeviceResources(DXGI_FORMAT backBufferFormat, DXGI_FORMAT depthBufferFormat, UINT backBufferCount,
	...
	: m_backBufferIndex(0), m_fenceValues{}, m_rtvDescriptorSize(0), m_screenViewport{}, m_scissorRect{},
	  ...
	  m_dxgiFactoryFlags(0), m_outputSize{0, 0, 1, 1}, m_colorSpace(DXGI_COLOR_SPACE_RGB_FULL_G22_NONE_P709), m_options(flags),
...
void DeviceResources::CreateDeviceResources()
{
	...
	D3D12_FEATURE_DATA_FEATURE_LEVELS featLevels = {static_cast<UINT>(std::size(s_featureLevels)),
													s_featureLevels,
													D3D_FEATURE_LEVEL_11_0};
	...
	auto** cc = m_commandQueue.ReleaseAndGetAddressOf();
	CheckHR(m_d3dDevice->CreateCommandQueue(&queueDesc, GetIID(cc), GetPPV(cc)));
	...
	auto** rtvDh = m_rtvDescriptorHeap.ReleaseAndGetAddressOf();
	CheckHR(m_d3dDevice->CreateDescriptorHeap(&rtvDescriptorHeapDesc, GetIID(rtvDh), GetPPV(rtvDh)));
	...
	if (m_depthBufferFormat != DXGI_FORMAT_UNKNOWN)
	{
		...
		auto** dsvDh = m_dsvDescriptorHeap.ReleaseAndGetAddressOf();
		CheckHR(m_d3dDevice->CreateDescriptorHeap(&dsvDescriptorHeapDesc, GetIID(dsvDh), GetPPV(dsvDh)));
	...
	CheckHR(m_d3dDevice->CreateCommandList(0,
...
void DeviceResources::CreateWindowSizeDependentResources()
{
	...
	else
	{
		...
		CheckHR(m_dxgiFactory->CreateSwapChainForHwnd(m_commandQueue.Get(),
													  m_window,
													  &swapChainDesc,
													  &fsSwapChainDesc,
													  nullptr,
		...
		CheckHR(swapChain.As(&m_swapChain));

		// This class does not support exclusive full-screen mode and prevents DXGI from responding to the ALT+ENTER shortcut
		CheckHR(m_dxgiFactory->MakeWindowAssociation(m_window, DXGI_MWA_NO_ALT_ENTER));
	}
	...
	if (m_depthBufferFormat != DXGI_FORMAT_UNKNOWN)
	{
		...
		CheckHR(m_d3dDevice->CreateCommittedResource(&depthHeapProperties,
...
void DeviceResources::Prepare(D3D12_RESOURCE_STATES beforeState, D3D12_RESOURCE_STATES afterState)
{
	...
	CheckHR(m_commandAllocators[m_backBufferIndex]->Reset());
	CheckHR(m_commandList->Reset(m_commandAllocators[m_backBufferIndex].Get(), nullptr));
...
{
	if (m_commandQueue && m_fence && m_fenceEvent != nullptr && m_fenceEvent != INVALID_HANDLE_VALUE)
	{
...
void DeviceResources::MoveToNextFrame()
{
	...
	CheckHR(m_commandQueue->Signal(m_fence.Get(), currentFenceValue));
	...
	{
		CheckHR(m_fence->SetEventOnCompletion(m_fenceValues[m_backBufferIndex], m_fenceEvent));
		std::ignore = WaitForSingleObjectEx(m_fenceEvent, INFINITE, FALSE);
	}
...
void DeviceResources::GetAdapter(IDXGIAdapter1** ppAdapter)
{
	*ppAdapter = nullptr;
	...
	if (!adapter)
	{
		for (UINT adapterIndex = 0; SUCCEEDED(m_dxgiFactory->EnumAdapters1(adapterIndex, adapter.ReleaseAndGetAddressOf()));
			 ++adapterIndex)
		{
			DXGI_ADAPTER_DESC1 desc;
			CheckHR(adapter->GetDesc1(&desc));
...
void DeviceResources::UpdateColorSpace()
{
	...
	if (m_swapChain)
	{
		...
		for (UINT adapterIndex = 0; SUCCEEDED(m_dxgiFactory->EnumAdapters(adapterIndex, adapter.ReleaseAndGetAddressOf()));
			 ++adapterIndex)
		{
	...
	{
		CheckHR(m_swapChain->SetColorSpace1(colorSpace));
	}
}
