...
public:
	...
	explicit DxDescriptorPile(ID3D12Device2* device, D3D12_DESCRIPTOR_HEAP_TYPE type, D3D12_DESCRIPTOR_HEAP_FLAGS flags,
							  size_t capacity, IndexType reserve = 0);
	explicit DxDescriptorPile(ID3D12Device2* device, size_t count, IndexType reserve = 0);
	...
	IndexType Allocate();
	void	  AllocateRange(size_t numDescriptors, IndexType& start, IndexType& end);
	void	  Free(IndexType index, size_t numDescriptors = 1);
